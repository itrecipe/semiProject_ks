...
myButton.addEventListener('click', function () {
    ...
    const car_save = JSON.parse(localStorage.getItem('st')) || [];

    //car_save.length 값을 디버깅
    console.log(car_save.length);

    if (car_save.length > 0) { // 저장된 차량 정보 배열이 비어있지 않은지 확인

        //저장된 차량 정보의 대한 배열을 순회하며 각각의 차량 정보를 화면에 표현한다.
        for (let i = 0; i < car_save.length; i++) {
            const carInfo = car_save[i]; // 현재 저장된 차량의 정보를 가져온다.
